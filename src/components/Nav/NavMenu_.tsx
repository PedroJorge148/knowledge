...
import { Books, User, UserCircle } from 'phosphor-react';

export function NavMenu_() {
  ...
  return (
    <nav className="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900">
      ...
      <a href="https://flowbite.com/" className="flex items-center">
        <Books weight="duotone" size={25} className="mr-3 sm:h-9 text-brand-500" />
      ...
      <div className="flex items-center md:order-2">
          <button 
            ...
            onClick={() => setUserNavbarOpen(!userNavbarOpen)}
          >
            <span className="sr-only">Open user menu</span>
            {/* <img className="w-8 h-8 rounded-full" src="/docs/images/people/profile-picture-3.jpg" alt="user photo"> */}
            <UserCircle weight="duotone" size={30} className="rounded-full" />
          </button>
          {/* <!-- Dropdown menu --> */}
          <div className={ (!userNavbarOpen? "hidden": "") + " z-50 my-4 text-base list-none bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600 block "}id="user-dropdown" data-popper-reference-hidden="" data-popper-escaped="" data-popper-placement="bottom">
          {/* style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(648px, 82px);" */}
